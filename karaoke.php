...
<body>
    ...
    <!-- Search for a Song -->
    <h2>Enter Your Name and Search for a Song:</h2>

    <form action="" method="POST">  
        <input type="text" id="userName" name="userName" required=true placeholder="Put your name" value="<?PHP if(isset($_POST['userName'])) echo htmlspecialchars($_POST['userName']); ?>">
        <input type="number" id="dollarAmount" name="dollarAmount" max=9000 placeholder="Enter $$$ for Priority" value="<?PHP if(isset($_POST['dollarAmount'])) echo htmlspecialchars($_POST['dollarAmount']); ?>"><br>
        <br>
        <label for="searchTerm">Search by artist, song title, or contributor (or nothing for all songs)</label><br>
    ...
    <!-- Only show rest of form if user enters name -->
    ...
    $sql_new_user = "SELECT User_ID FROM Users
                WHERE User_Name = ?";

    try {
        $statement = $pdo->prepare($sql_new_user);
        $statement->execute([$name]);
        // These variable assignments could be refined
    ...
    if ($dollar_Amount > 0 && $file_name != "" && $name != "") {
    ...
    } else if ($dollar_Amount == 0 && $file_name != "" && $name != "") {
    ...
    $sql = "SELECT
                ...
                Kar_File.File_Name AS 'Filename'
            FROM
                Kar_File
            LEFT JOIN Song
    ...
    if (!empty($rows)) {
        ...
        // Submit button for song
        echo "<td><form action='karaoke.php' method='POST'>
            <input type='hidden' name='userName' value=$name>
		    <input type='hidden' name='dollarAmount' value=$dollar_Amount>
		    <input type='hidden' name='fileName' value='" . htmlspecialchars($col) . "'>
            <input type='hidden' name='searchTerm' value='" . htmlspecialchars($search_Term) . "'>
		    <input type='hidden' name='Select' value='Selected'>
		    <button type='submit'>SING THIS!</button>
        ...
        echo "<td><input type='radio' id='priority_{$row['Date_Time']}' name='fileName' value='" . htmlspecialchars($col) . "'></td>";
