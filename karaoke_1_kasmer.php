...
<body>
    ...
    <form action="" method="POST">
        ...
        <input type="text" id="searchTerm" name="searchTerm" required="true" placeholder="e.g. Kid Cudi" value="<?php if (isset($_POST['searchTerm'])) echo htmlspecialchars($_POST['searchTerm']); ?>"><br>
        <input type="hidden" name="formType" value="SEARCH">
        <input type="hidden" name="Column" value="Artist.Artist_Name">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["Column"])) {
            $allowed_columns = ["Artist.Artist_Name", "Kar_File.Extended_Title", "Song.Song_Name"];
            $sort_Column = in_array($_POST["Column"], $allowed_columns) ? $_POST["Column"] : "Artist.Artist_Name";
        }
    ...
    if (!empty($rows)) {
        ...
        foreach (array_keys($rows[0]) as $heading) {
            echo "<th><form action='karaoke.php' method='POST'>
                    ...
                    <button type='submit'>" . htmlspecialchars($heading) . "</button>
                  </form></th>";
        }
        echo "</tr>\n";
